module scheme
    ...
    use van_leer, only: &
            ...
            G_van_leer => G, &
            H_van_leer => H
    use viscous, only: setup_viscous, compute_viscous_fluxes, &
                       destroy_viscous
!   use ausm, only: &
...
!           get_residue_hlle => get_residue
    ...
    contains
      ...
      !             errmsg='Error: Unable to allocate memory for flux F.')
      !     call alloc(G, 1, imx-1, 1, jmx, 1, kmx-1, 1, n_var, &
      !             errmsg='Error: Unable to allocate memory for flux G.')
      !     call alloc(H, 1, imx-1, 1, jmx-1, 1, kmx, 1, n_var, &
      !             errmsg='Error: Unable to allocate memory for flux H.')
