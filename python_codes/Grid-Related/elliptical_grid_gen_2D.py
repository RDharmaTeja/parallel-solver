...
#                   round((y_limits[1] - y_limits[0]) / dy + 1), endpoint=True)
    ...
    x = np.linspace(x_limits[0], x_limits[1], 201)
                    
    y = np.linspace(y_limits[0], y_limits[1], 101)
...
def main_function():
    ...
    gridfile = 'plane-IB.txt'
    datafile = None
    opfilename = 'plane-IB.vtk'
    filecomment = 'Testing 2D grid for Immersed Boundary Method'
    bump = 'bottom'
